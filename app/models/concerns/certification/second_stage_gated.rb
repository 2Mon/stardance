  ...
  module SecondStageGated
    ...
    included do
      ...
      after_save :void_second_stage_review!,
                 if: -> { saved_change_to_status? && !approved? && !releasing_second_stage }
    end
    ...
    # feedback and reviewer_id.
    def return_from_second_stage!(reviewer:, feedback: nil)
      attrs = { status: :returned, decided_at: Time.current }
      attrs[:feedback] = feedback if feedback.present?
      attrs[:reviewer] = reviewer if reviewer
